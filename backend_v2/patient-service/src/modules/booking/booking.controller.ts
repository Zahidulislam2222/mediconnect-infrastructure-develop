...
const generateReceipt = (data: any): string => {
    const doc = new jsPDF();

    // Header
    doc.setFontSize(20);
    doc.text("MediConnect - Payment Receipt", 105, 20, { align: "center" });

    // Meta Info
    doc.setFontSize(12);
    doc.text(`Receipt ID: ${data.billId}`, 20, 40);
    doc.text(`Date: ${new Date().toLocaleDateString()}`, 20, 50);

    // Patient/Doctor Info
    doc.text(`Patient: ${data.patientName}`, 20, 70);
    doc.text(`Doctor: ${data.doctorName}`, 20, 80);
    doc.text(`Appointment Time: ${data.timeSlot}`, 20, 90);
    ...
    autoTable(doc, {
        ...
        body: [
            ['Consultation Fee', `$${data.amountPaid}`, 'PAID'],
            ['Booking Priority', data.priority, '-']
        ],
    });

    doc.text(`Total Paid: $${data.amountPaid}`, 150, 140);

    // Footer
    doc.setFontSize(10);
    doc.text("Thank you for choosing MediConnect.", 105, 180, { align: "center" });
...
export const createBooking = async (req: Request, res: Response) => {
    ...
    try {
        ...
        try {
            ...
            safeError("Payment Failed:", paymentError.message);
            // Release lock
        ...
        try {
            ...
            let receiptBase64 = "";
            try {
                receiptBase64 = generateReceipt({
                    billId: transactionId,
                    patientName, doctorName, timeSlot,
                    amountPaid: amountToCharge / 100,
                    priority
                });
            } catch (e) { safeError("Receipt Gen Failed", e); }
            ...
            res.status(200).json({
                ...
                priority,
                queueStatus: "WAITING",
                receiptData: receiptBase64 // Returning Base64 PDF
            });

        } catch (dbError: any) {
            safeError("Transaction Failed. Initiating Refund.", dbError);
            ...
            if (stripeInstance && paymentIntentId) {
                try { await stripeInstance.refunds.create({ payment_intent: paymentIntentId }); } catch (e) {
                    safeError("Refund failed dramatically:", e);
                }
            }
...
export const getAppointments = async (req: Request, res: Response) => {
    try {
        ...
        safeError("GetAppointments Error:", error);
            ...
            if (diffMinutes >= 30 && apt.patientArrived) {
                ...
                if (stripe && apt.paymentId && apt.paymentId !== "TEST_MODE") {
                    try {
                        ...
                        refundId = refund.id;
                    } catch (e) { safeError("Refund failed", e); }
                }
...
async function cancelAppointment(apt: any, newStatus: string, refundId: string) {
    try {
        ...
        safeError("Cancel update failed", e);
    }
}
